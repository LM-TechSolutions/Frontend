  ...
  return (
    <div className="p-6">
      ...
      {selectedDriver ? (
          ...
          <Card className="mb-6 shadow-sm">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div><p className="text-sm text-[#6B7280] mb-1">Driver Name</p><p className="font-semibold text-[#111827]">{selectedDriver.name}</p></div>
                <div><p className="text-sm text-[#6B7280] mb-1">Phone</p><p className="font-semibold text-[#111827]">{selectedDriver.phone}</p></div>
                <div><p className="text-sm text-[#6B7280] mb-1">Vehicle</p><p className="font-semibold text-[#111827]">{selectedDriver.vehicleType ?? '—'}</p></div>
                <div><p className="text-sm text-[#6B7280] mb-1">License Plate</p><p className="font-semibold text-[#111827]">{selectedDriver.licensePlate}</p></div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-baseline gap-3">
                    <p className="text-5xl font-bold text-[#111827]">{currentBalance}</p>
                ...
                <Dialog open={refillOpen} onOpenChange={setRefillOpen}>
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    ...
                    <div className="space-y-4 py-4">
                      <div className="space-y-2"><Label>Driver</Label><p className="font-medium text-[#111827]">{selectedDriver.name}</p></div>
                      <div className="space-y-2"><Label>Current Balance</Label><p className="text-2xl font-bold text-[#111827]">{currentBalance} coupons</p></div>
                      ...
                      {refillAmount && parseInt(refillAmount) > 0 && (
                        <div className="bg-[#F9FAFB] p-3 rounded-lg"><p className="text-sm text-[#6B7280] mb-1">New Balance</p><p className="text-xl font-bold text-[#00BDC3]">{currentBalance + parseInt(refillAmount)} coupons</p></div>
                      )}
                    </div>
                    ...
                    {txLoading ? (
                      ...
                      transactions.map((t) => (
                        <TableRow key={t.id} className="hover:bg-[#F9FAFB]">
                          <TableCell className="text-sm text-[#6B7280]">{format(new Date(t.createdAt), 'MMM dd, yyyy HH:mm')}</TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-sm text-[#6B7280]">{t.notes ?? '-'}</TableCell>
                          <TableCell className="font-medium text-[#111827]">{t.balance} coupons</TableCell>
                        </TableRow>
                      ))
                    ) : (
                      <TableRow><TableCell colSpan={5} className="text-center py-8 text-[#6B7280]">No transactions found for this driver</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
        ...
        <div className="text-center py-12"><p className="text-[#6B7280]">No drivers yet — add a driver first.</p></div>
      )}
    </div>
  );
}
